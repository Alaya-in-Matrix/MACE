...
this work, we use squared-exponential ARD kernel, and a constant mean function
$m(\bm{x}) = \mu_0$ for all our experiments. By default, we assume the
objective function evaluations are noisy and let GP to infer the noise level
$\sigma_n$. The introduction of the $\sigma_n$ level also helps to improve the
numerical stability.
