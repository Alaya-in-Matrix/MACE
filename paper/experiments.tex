...
batch size is set to $B = 4$. The number initial random sampling is set to
...
The mean convergence plot is given in Figure~\ref{fig:resClassE}, the mean and
standard deviation of the final FOM values are listed in
Table~\ref{tab:result_PA}. We can see that the MACE outperformed the BLCB and
EI-LP in both sequential and batch mode.  For the batch runs, the MACE
convergences fastest among the three algorithms, while the sequential MACE
algorithm while the sequential MACE has similar performance as the batch EI-LP
method.
